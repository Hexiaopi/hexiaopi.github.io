<template><div><h2 id="认证" tabindex="-1"><a class="header-anchor" href="#认证" aria-hidden="true">#</a> 认证</h2>
<blockquote>
<p>我们有多种途径来保障应用的安全，例如：网咯隔离、设置防火墙、设置IP黑名单等。而<strong>认证</strong>是软件层面保护应用安全的一种措施。</p>
<p>当前有四种认证方式，分别是：<code v-pre>Basic</code>、<code v-pre>Digest</code>、<code v-pre>OAuth</code>和<code v-pre>Bearer</code></p>
</blockquote>
<h3 id="basic认证" tabindex="-1"><a class="header-anchor" href="#basic认证" aria-hidden="true">#</a> Basic认证</h3>
<blockquote>
<p>Basic认证（基础认证）是一种简单的认证方式。将<code v-pre>用户名:密码</code>进行base64编码，放到HTTP Authorization Header中。</p>
...
<h3 id="digest认证" tabindex="-1"><a class="header-anchor" href="#digest认证" aria-hidden="true">#</a> Digest认证</h3>
<blockquote>
...
<p>摘要认证的过程如下图所示：<br>
<img src="@source/system-design/security/images/digest.png" alt="digest" loading="lazy"></p>
<ol>
<li>客户端请求服务端资源</li>
<li>在客户端能够证明它知道密码从而确认其身份之前，服务端认证失败，返回401 Unauthorized，并返回WWW-Authenticate头，里面包含认证需要的信息。</li>
<li>客户端根据WWW-Authenticate头中的信息，选择加密算法，并使用密码随机数<code v-pre>nonce</code>，计算出密码摘要<code v-pre>response</code>，并再次请求服务端。</li>
<li>服务器将客户端提供的密码摘要与服务器内部计算出的摘要进行对比。如果匹配，就说明客户端知道密码，认证通过，并返回一些与授权会话相关的附加信息，放在<code v-pre>Authorization-Info</code>中。</li>
</ol>
<p>其中涉及到的一些参数说明：</p>
<table>
<thead>
...
<h3 id="oauth认证" tabindex="-1"><a class="header-anchor" href="#oauth认证" aria-hidden="true">#</a> OAuth认证</h3>
<blockquote>
<p>OAuth（开放授权）是一个开放的授权标准，允许用户让第三方应用访问该用户在某一Web服务上存储的私密资源（例如：照片、视频、音频等），而无需将用户名和密码提供给第三方应用。</p>
</blockquote>
<p>阮老师解释OAuth2.0非常形象，一看就懂<br>
<a href="https://www.ruanyifeng.com/blog/2019/04/oauth_design.html" target="_blank" rel="noopener noreferrer">OAuth 2.0 的一个简单解释<ExternalLinkIcon/></a></p>
<p><a href="https://www.ruanyifeng.com/blog/2019/04/oauth-grant-types.html" target="_blank" rel="noopener noreferrer">OAuth 2.0 的四种方式<ExternalLinkIcon/></a></p>
<p><a href="https://www.ruanyifeng.com/blog/2019/04/github-oauth.html" target="_blank" rel="noopener noreferrer">GitHub OAuth 第三方登录示例教程<ExternalLinkIcon/></a></p>
<p>OAuth当前是2.0版本，分为四种授权方式：密码式、隐藏式、凭借式和授权码模式。</p>
<h4 id="密码式" tabindex="-1"><a class="header-anchor" href="#密码式" aria-hidden="true">#</a> 密码式</h4>
<p>流程如下：</p>
<ol>
...
<p>特点：用户需要高度信任网站A</p>
<h4 id="隐藏式" tabindex="-1"><a class="header-anchor" href="#隐藏式" aria-hidden="true">#</a> 隐藏式</h4>
<p>流程如下：</p>
<ol>
...
<h4 id="凭借式" tabindex="-1"><a class="header-anchor" href="#凭借式" aria-hidden="true">#</a> 凭借式</h4>
<ol>
...
<h4 id="授权码模式" tabindex="-1"><a class="header-anchor" href="#授权码模式" aria-hidden="true">#</a> 授权码模式</h4>
<p>流程如下：</p>
<ol>
...
<h3 id="bearer认证" tabindex="-1"><a class="header-anchor" href="#bearer认证" aria-hidden="true">#</a> Bearer认证</h3>
<blockquote>
<p>Bearer认证也称为令牌认证，是一种HTTP身份验证方法，Bearer认证的核心是bearer token。bearer token是一个加密字符串，通常由服务端根据秘钥生成。客户端在请求服务端时，必须在请求头中包含Authorization: Bearer <code v-pre>&lt;token&gt;</code>，服务端收到请求后，解析出token，并校验token的合法性。</p>
</blockquote>
<p>当前最流行的token编码方式是JSON Web Token(JWT)</p>
<h4 id="基于jwt的token认证" tabindex="-1"><a class="header-anchor" href="#基于jwt的token认证" aria-hidden="true">#</a> 基于JWT的Token认证</h4>
<blockquote>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>Header</strong></p>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>$ echo -n '{"typ":"JWT","alg":"HS256"}'|base64
eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>在某些场景下，可能还会有kid选项，用来标识一个秘钥ID，例如：</p>
<div class="language-json line-numbers-mode" data-ext="json"><pre v-pre class="language-json"><code><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>Payload</strong></p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>将payload进行base64编码：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>$ echo -n '{"aud":"iam.authz.marmotedu.com","exp":1604158987,"iat":1604151787,"iss":"iamctl","nbf":1604151787}'|base64
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这些预定义的字段并不要求强制使用，此外，我们还可以自定义私有字段，一般会把包含用户信息的数据放到payload中。注意，不要添加敏感信息，此外私有声明是客户端和服务端所共同定义的声明。</p>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>HMACSHA256(base64UrlEncode(header)+"."+base64UrlEncode(payload),secret)
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>得到 Signature。并且，对比加密后的 Signature 和收到的 Signature 是否相同，如果相同则验证通过，不相同则返回 HTTP 401 Unauthorized 的错误。</p>
</div></template>
