<template><div><figure><img src="http://cdn.cjhe.top/blog/map.png" alt="map" tabindex="0" loading="lazy"><figcaption>map</figcaption></figure>
<!-- more -->
<blockquote>
<p>map是Go语言提供的一种抽象数据类型，它表示一组无序的键值对(key-value)</p>
</blockquote>
<h2 id="声明" tabindex="-1"><a class="header-anchor" href="#声明" aria-hidden="true">#</a> 声明</h2>
<p><strong>关键字定义</strong>：<code v-pre>map</code></p>
<p><strong>零值</strong>：<code v-pre>nil</code></p>
<div class="language-go line-numbers-mode" data-ext="go"><pre v-pre class="language-go"><code><span class="token keyword">var</span> m <span class="token keyword">map</span><span class="token punctuation">[</span><span class="token builtin">string</span><span class="token punctuation">]</span><span class="token builtin">int</span>
...
<p>map对于<code v-pre>value</code>的类型没有限制，但是对<code v-pre>key</code>的类型有严格的要求：<strong>key的类型必须支持&quot;==&quot;和&quot;!=&quot;两个操作符的数据类型</strong>，因此：函数、map、切片不能作为key类型。</p>
</div>
<div class="hint-container warning">
<p class="hint-container-title">注意</p>
<p>向<code v-pre>nil</code>（未初始化）的map类型里添加数据会导致<code v-pre>panic</code><Badge text="注意" type="warning"/></p>
</div>
...
<li>查询不存在的key，如果未使用<strong>comma ok</strong>语法，则会返回value类型对应的零值</li>
</ul>
<h2 id="底层原理" tabindex="-1"><a class="header-anchor" href="#底层原理" aria-hidden="true">#</a> 底层原理</h2>
<p>参见<code v-pre>$GOROOT/src/runtime/map.go</code></p>
<h3 id="map的数据结构" tabindex="-1"><a class="header-anchor" href="#map的数据结构" aria-hidden="true">#</a> map的数据结构</h3>
<div class="language-go line-numbers-mode" data-ext="go"><pre v-pre class="language-go"><code><span class="token keyword">type</span> hmap <span class="token keyword">struct</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="bucket的数据结构" tabindex="-1"><a class="header-anchor" href="#bucket的数据结构" aria-hidden="true">#</a> bucket的数据结构</h3>
<div class="language-go line-numbers-mode" data-ext="go"><pre v-pre class="language-go"><code><span class="token comment">// a bucket for a Go map</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="@source/go-language/base/images/map.png" alt="map" tabindex="0" loading="lazy"><figcaption>map</figcaption></figure>
<p>key和value分开存储，目的是为了减少内存对齐带来的内存浪费，以map[int8]int64为例：</p>
<figure><img src="@source/go-language/base/images/map-memory.png" alt="map-memory" tabindex="0" loading="lazy"><figcaption>map-memory</figcaption></figure>
<p>每个bucket可以存储8个键值对，超过就会创建一个新的bucket，通过overflow链接</p>
<p>当有两个或以上数量的键被<code v-pre>Hash</code>到了同一个bucket时，我们称这些键发生了冲突，Go语言使用链表解决键冲突。</p>
<p>hash冲突并不是好事情，它降低了存取效率，好的Hash算法可以保证Hash值得随机性，但无论使用哪种Hash算法，冲突终究不可避免，当冲突较多的时候就需要采取一些措施来减少冲突。</p>
<h3 id="负载因子loadfactor" tabindex="-1"><a class="header-anchor" href="#负载因子loadfactor" aria-hidden="true">#</a> 负载因子LoadFactor</h3>
<blockquote>
...
<p>map的扩缩容的主要区别在于hmap.B的容量大小改变，而缩容由于hmap.B压根不变，内存占用依然存在。这就导致在删除元素时，并不会释放内存，使得分配的总内存不断增加，如果不注意，内存就很容易爆了。<Badge text="注意" type="warning"/><br>
<a href="https://eddycjy.com/posts/go/map-reset/" target="_blank" rel="noopener noreferrer">Go map 如何缩容？<ExternalLinkIcon/></a></p>
<h4 id="增量扩容" tabindex="-1"><a class="header-anchor" href="#增量扩容" aria-hidden="true">#</a> 增量扩容</h4>
<blockquote>
...
<figure><img src="@source/go-language/base/images/map-expand.png" alt="map-expand" tabindex="0" loading="lazy"><figcaption>map-expand</figcaption></figure>
<h4 id="等量扩容" tabindex="-1"><a class="header-anchor" href="#等量扩容" aria-hidden="true">#</a> 等量扩容</h4>
<blockquote>
<p>用于解决一个bucket链接很多bucket，overflow数量过高</p>
<p>bucket数组大小不变，重新排列键值对，分散到不同的bucket数组中</p>
<p>这样就提高了访问效率</p>
</blockquote>
<h3 id="增删改查" tabindex="-1"><a class="header-anchor" href="#增删改查" aria-hidden="true">#</a> 增删改查</h3>
<h4 id="查找过程" tabindex="-1"><a class="header-anchor" href="#查找过程" aria-hidden="true">#</a> 查找过程</h4>
<ul>
<li>根据key计算hash值；</li>
...
<h4 id="添加过程" tabindex="-1"><a class="header-anchor" href="#添加过程" aria-hidden="true">#</a> 添加过程</h4>
<ul>
<li>根据key计算hash值；</li>
<li>取hash值低位与hmap.B取模来确定bucket的位置；</li>
<li>查找该key是否存在
...
<h4 id="删除过程" tabindex="-1"><a class="header-anchor" href="#删除过程" aria-hidden="true">#</a> 删除过程</h4>
<ul>
<li>查找key是否存在
<ul>
<li>存在，则删除；</li>
...
<h2 id="map注意事项" tabindex="-1"><a class="header-anchor" href="#map注意事项" aria-hidden="true">#</a> map注意事项</h2>
<h3 id="非并发读写安全的" tabindex="-1"><a class="header-anchor" href="#非并发读写安全的" aria-hidden="true">#</a> 非并发读写安全的</h3>
<div class="language-go line-numbers-mode" data-ext="go"><pre v-pre class="language-go"><code><span class="token keyword">package</span> main
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>运行报错：<code v-pre>fatal error: concurrent map iteration and map write</code></p>
<div class="hint-container tip">
<p class="hint-container-title">提示</p>
<p>Go 在 map 的实现中增加了读写检查机制，一旦发现读写冲突立即触发 panic</p>
<p>如果想使用并发的map：</p>
<ul>
<li>可以使用Go 1.9版本中引入支持并发写安全的<code v-pre>sync.Map</code>类型；</li>
<li>可以使用map时增加锁；</li>
</ul>
</div>
<h3 id="无法通过索引直接获取value的地址" tabindex="-1"><a class="header-anchor" href="#无法通过索引直接获取value的地址" aria-hidden="true">#</a> 无法通过索引直接获取value的地址</h3>
<div class="language-go line-numbers-mode" data-ext="go"><pre v-pre class="language-go"><code><span class="token keyword">package</span> main
...
<h3 id="尽量使用cap参数初始化map" tabindex="-1"><a class="header-anchor" href="#尽量使用cap参数初始化map" aria-hidden="true">#</a> 尽量使用cap参数初始化map</h3>
<div class="language-go line-numbers-mode" data-ext="go"><pre v-pre class="language-go"><code><span class="token keyword">package</span> main
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>go test -benchmem -bench=. cap_benchmark_test.go           
goos: darwin
goarch: amd64
