<template><div><p>Go语言sync包提供了两种锁：</p>
<ul>
<li>互斥锁 Mutex:
<ul>
<li>加锁时，其他加锁协程均阻塞</li>
...
<!-- more -->
<nav class="table-of-contents"><ul><li><router-link to="#互斥锁-mutex">互斥锁 Mutex</router-link><ul><li><router-link to="#mutex数据结构">Mutex数据结构</router-link></li><li><router-link to="#mutex的方法">Mutex的方法</router-link></li><li><router-link to="#自旋过程">自旋过程</router-link></li><li><router-link to="#mutex注意事项">Mutex注意事项</router-link></li></ul></li><li><router-link to="#读写锁-rwmutex">读写锁 RWMutex</router-link><ul><li><router-link to="#rwmutex数据结构">RWMutex数据结构</router-link></li><li><router-link to="#rwmutex的方法">RWMutex的方法</router-link></li><li><router-link to="#rwmutex的细节分析">RWMutex的细节分析</router-link></li></ul></li><li><router-link to="#参考资料">参考资料</router-link></li></ul></nav>
<h2 id="互斥锁-mutex" tabindex="-1"><a class="header-anchor" href="#互斥锁-mutex" aria-hidden="true">#</a> 互斥锁 Mutex</h2>
<h3 id="mutex数据结构" tabindex="-1"><a class="header-anchor" href="#mutex数据结构" aria-hidden="true">#</a> Mutex数据结构</h3>
<p>参见<code v-pre>$GOROOT/src/sync/mutex.go</code></p>
<div class="language-go line-numbers-mode" data-ext="go"><pre v-pre class="language-go"><code><span class="token keyword">type</span> Mutex <span class="token keyword">struct</span><span class="token punctuation">{</span>
  state <span class="token builtin">int32</span>
  sema  <span class="token builtin">uint32</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>其中：</p>
<ul>
<li>state: 表示互斥锁的状态，比如是否被锁定等；</li>
<li>sema: 表示信号量，协程阻塞等待该信号量，解锁的协程释放信号量从而唤醒等待信号量的协程；</li>
</ul>
<p>Mutex.state虽然是<code v-pre>int32</code>数据类型，内部实现时把该变量分成四份，用于记录Mutex的四种状态</p>
<figure><img src="@source/go-language/base/images/mutex.png" alt="mutex" tabindex="0" loading="lazy"><figcaption>mutex</figcaption></figure>
<ul>
...
<h3 id="mutex的方法" tabindex="-1"><a class="header-anchor" href="#mutex的方法" aria-hidden="true">#</a> Mutex的方法</h3>
<p>Mutex对外提供两个方法：</p>
<ul>
<li>Lock(): 加锁方法</li>
<li>Unlock(): 解锁方法</li>
</ul>
<h4 id="单个协程的加锁释放过程" tabindex="-1"><a class="header-anchor" href="#单个协程的加锁释放过程" aria-hidden="true">#</a> 单个协程的加锁释放过程</h4>
<p>Mutex内部<code v-pre>state</code>表示如下：</p>
<figure><img src="@source/go-language/base/images/mutex-simple.png" alt="mutex-simple" tabindex="0" loading="lazy"><figcaption>mutex-simple</figcaption></figure>
<h4 id="两个协程的加锁释放过程" tabindex="-1"><a class="header-anchor" href="#两个协程的加锁释放过程" aria-hidden="true">#</a> 两个协程的加锁释放过程</h4>
<p>Mutex内部<code v-pre>state</code>表示如下：</p>
<figure><img src="@source/go-language/base/images/mutex-complex.png" alt="mutex-comple" tabindex="0" loading="lazy"><figcaption>mutex-comple</figcaption></figure>
<p>上面介绍Mutex加锁和解锁只使用内部<code v-pre>state</code>标记：<code v-pre>Waiter</code>和<code v-pre>Locked</code>两个，另外两个标记就不得不提自旋过程了。</p>
<h3 id="自旋过程" tabindex="-1"><a class="header-anchor" href="#自旋过程" aria-hidden="true">#</a> 自旋过程</h3>
<blockquote>
<p>自旋过程是：加锁时，如果当前<code v-pre>Locked</code>为1，则说明当前该锁由其他协程持有，尝试加锁的协程并不是马上转入阻塞，而是会持续地探测<code v-pre>Locked</code>是否变为0，这个过程就是自旋过程。</p>
</blockquote>
...
<p>如果自旋过程中获得锁，那么之前被阻塞的协程将无法获得锁，之前阻塞的协程一直获取不到锁（超过1ms）将进入<strong>饥饿</strong>状态<Badge text="注意" type="warning"/>。为了避免这个问题，Go1.8版本Mutex增加新的状态<code v-pre>Starving</code>，这个状态下不会自旋，一旦协程释放锁，就会唤醒一个等待加锁协程。</p>
<p>如果此时有两个协程，一个在加锁，另一个在解锁，在加锁的过程中可能处于自旋，此时会把<code v-pre>Woken</code>标记为1，用于告知解锁协程，不必释放信号量，它很快会拿到锁。</p>
<h3 id="mutex注意事项" tabindex="-1"><a class="header-anchor" href="#mutex注意事项" aria-hidden="true">#</a> Mutex注意事项</h3>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>2023/03/12 19:23:45 g1: lock foo ok
...
<p>sync包中类型的实例在被复制得到的副本后将脱离原实例的控制范围，因此推荐传指针的方式<Badge text="注意" type="warning"/>，保证使用的锁是同一实例。</p>
<h2 id="读写锁-rwmutex" tabindex="-1"><a class="header-anchor" href="#读写锁-rwmutex" aria-hidden="true">#</a> 读写锁 RWMutex</h2>
<h3 id="rwmutex数据结构" tabindex="-1"><a class="header-anchor" href="#rwmutex数据结构" aria-hidden="true">#</a> RWMutex数据结构</h3>
<p>参见<code v-pre>$GOROOT/src/sync/mutex.go</code></p>
<div class="language-go line-numbers-mode" data-ext="go"><pre v-pre class="language-go"><code><span class="token keyword">type</span> RWMutex <span class="token keyword">struct</span><span class="token punctuation">{</span>
  w           Mutex
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>其中：</p>
<ul>
...
<h3 id="rwmutex的方法" tabindex="-1"><a class="header-anchor" href="#rwmutex的方法" aria-hidden="true">#</a> RWMutex的方法</h3>
<ul>
<li>RLock(): 加读锁
<ul>
...
<figure><img src="@source/go-language/base/images/rwmutex-logic.png" alt="rwmutex" tabindex="0" loading="lazy"><figcaption>rwmutex</figcaption></figure>
<h3 id="rwmutex的细节分析" tabindex="-1"><a class="header-anchor" href="#rwmutex的细节分析" aria-hidden="true">#</a> RWMutex的细节分析</h3>
<h4 id="写操作是如何阻止写操作的" tabindex="-1"><a class="header-anchor" href="#写操作是如何阻止写操作的" aria-hidden="true">#</a> 写操作是如何阻止写操作的？</h4>
<p>读写锁RWMutex包含一个互斥锁w，获取写锁必须先获取该互斥锁，如果互斥锁已被其他协程持有，则只能阻塞等待。</p>
<h4 id="写操作是如何阻止读操作的" tabindex="-1"><a class="header-anchor" href="#写操作是如何阻止读操作的" aria-hidden="true">#</a> 写操作是如何阻止读操作的？</h4>
...
<h4 id="读操作是如何阻止写操作的" tabindex="-1"><a class="header-anchor" href="#读操作是如何阻止写操作的" aria-hidden="true">#</a> 读操作是如何阻止写操作的？</h4>
<p>读锁会先将RWMutex.readerCount加1，写锁协程检测到readerCount不为0，则会阻塞自己。</p>
<h4 id="写锁为什么不会被饿死" tabindex="-1"><a class="header-anchor" href="#写锁为什么不会被饿死" aria-hidden="true">#</a> 写锁为什么不会被<strong>饿死</strong>？</h4>
...
<p><a href="https://book.douban.com/subject/35144587/" target="_blank" rel="noopener noreferrer">Go专家编程<ExternalLinkIcon/></a></p>
</div></template>
