<template><div><figure><img src="https://refactoringguru.cn/images/patterns/content/factory-method/factory-method-zh-2x.png" alt="factory-method" tabindex="0" loading="lazy"><figcaption>factory-method</figcaption></figure>
<!-- more -->
<h2 id="什么是工厂方法模式" tabindex="-1"><a class="header-anchor" href="#什么是工厂方法模式" aria-hidden="true">#</a> 什么是工厂方法模式</h2>
<p>工厂方法模式(Factory Method Pattern)也叫虚拟构造器(Virtual Constructor)模式，由于简单工厂模式违背了开闭原则，而工厂方法模式对简单工厂模式进一步抽象，其好处是可以使系统在不修改原来代码的情况下引进新的产品，即满足开闭原则。</p>
<h3 id="包含哪些角色" tabindex="-1"><a class="header-anchor" href="#包含哪些角色" aria-hidden="true">#</a> 包含哪些角色</h3>
<figure><img src="@source/design-pattern/images/factory-method.png" alt="工厂方法模式" tabindex="0" loading="lazy"><figcaption>工厂方法模式</figcaption></figure>
<ul>
<li>
...
<div class="language-go line-numbers-mode" data-ext="go"><pre v-pre class="language-go"><code><span class="token keyword">package</span> factory_method
...
<ul>
<li>遵循了开闭原则，扩展性极强。增加新的产品类时，只需增加对应的工厂类，不需要修改原有代码。</li>
...
<h3 id="简单工厂和工厂方法对比" tabindex="-1"><a class="header-anchor" href="#简单工厂和工厂方法对比" aria-hidden="true">#</a> 简单工厂和工厂方法对比</h3>
<ul>
<li>简单工厂模式只有一个工厂类，用来创建同一等级结构中的任意产品，而工厂方法模式有多个工厂类，每个工厂类负责创建一个具体的产品。</li>
<li>简单工厂模式不支持扩展增加产品，如果要增加新的产品类，就要修改工厂类的代码，违反了开闭原则。而工厂方法模式支持扩展增加产品，只要增加相应的工厂类即可，不需要修改原有代码。</li>
<li>简单工厂模式使用静态方法来创建对象，这样无法形成基于继承的结构。而工厂方法模式使用抽象类和多态来创建对象，这样可以形成基于继承的结构。</li>
</ul>
</div></template>
