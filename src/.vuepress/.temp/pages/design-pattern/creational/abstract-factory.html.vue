<template><div><figure><img src="https://refactoringguru.cn/images/patterns/content/abstract-factory/abstract-factory-zh-2x.png" alt="abstract-factory" tabindex="0" loading="lazy"><figcaption>abstract-factory</figcaption></figure>
<!-- more -->
<h2 id="什么是抽象工厂模式" tabindex="-1"><a class="header-anchor" href="#什么是抽象工厂模式" aria-hidden="true">#</a> 什么是抽象工厂模式</h2>
...
<figure><img src="@source/design-pattern/images/abstract-factory.png" alt="抽象工厂模式" tabindex="0" loading="lazy"><figcaption>抽象工厂模式</figcaption></figure>
<ul>
<li>
...
<div class="language-go line-numbers-mode" data-ext="go"><pre v-pre class="language-go"><code><span class="token keyword">package</span> abstract_factory
...
<h3 id="简单工厂、工厂方法和抽象工厂的区别" tabindex="-1"><a class="header-anchor" href="#简单工厂、工厂方法和抽象工厂的区别" aria-hidden="true">#</a> 简单工厂、工厂方法和抽象工厂的区别</h3>
<ul>
<li>简单工厂模式：一个工厂方法创建不同类型的对象，根据传入的参数判断并返回相应的具体对象。这种模式简单易用，但是不符合开闭原则，每次增加新的产品都需要修改工厂方法。</li>
<li>工厂方法模式：一个具体的工厂类负责创建一个具体对象类型，每个产品对应一个工厂类。这种模式符合开闭原则，可以方便地增加新的产品和工厂，但是会导致类的数量增多。</li>
<li>抽象工厂模式：一个具体的工厂类负责创建一系列相关对象，每个产品族对应一个工厂类。这种模式可以隔离具体类的生产，保证客户端始终只使用同一个产品族中的对象，但是扩展新的产品或者产品族很困难，需要修改抽象工厂和所有的具体工厂。</li>
</ul>
</div></template>
