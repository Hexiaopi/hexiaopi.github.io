<template><div><h2 id="事务的特性" tabindex="-1"><a class="header-anchor" href="#事务的特性" aria-hidden="true">#</a> 事务的特性</h2>
<ul>
...
<h2 id="事务的实现" tabindex="-1"><a class="header-anchor" href="#事务的实现" aria-hidden="true">#</a> 事务的实现</h2>
<ul>
...
<h2 id="并发事务带来的问题" tabindex="-1"><a class="header-anchor" href="#并发事务带来的问题" aria-hidden="true">#</a> 并发事务带来的问题</h2>
...
<figure><img src="@source/database/mysql/images/dirty-read.png" alt="dirty-read" tabindex="0" loading="lazy"><figcaption>dirty-read</figcaption></figure>
...
<figure><img src="@source/database/mysql/images/non-repeatable-read.png" alt="non-repeatable-read" tabindex="0" loading="lazy"><figcaption>non-repeatable-read</figcaption></figure>
...
<figure><img src="@source/database/mysql/images/phantom-read.png" alt="phantom-read" tabindex="0" loading="lazy"><figcaption>phantom-read</figcaption></figure>
<h2 id="事务的隔离级别" tabindex="-1"><a class="header-anchor" href="#事务的隔离级别" aria-hidden="true">#</a> 事务的隔离级别</h2>
<p>针对上面三种问题，分别有四种隔离级别：</p>
<h3 id="未提交读-read-uncommitted-ru" tabindex="-1"><a class="header-anchor" href="#未提交读-read-uncommitted-ru" aria-hidden="true">#</a> 未提交读(read uncommitted，RU)</h3>
<blockquote>
<p>未提交读，是指一个事务还未提交时，它做的变更就能被别的事务看到。即<strong>没有提交却可以读</strong></p>
</blockquote>
<h3 id="提交读-read-committed-rc" tabindex="-1"><a class="header-anchor" href="#提交读-read-committed-rc" aria-hidden="true">#</a> 提交读(read committed，RC)</h3>
<blockquote>
<p>提交读，是指一个事务提交之后，它做的变更才会被其他事务看到。即<strong>提交才可以读</strong></p>
</blockquote>
<p>👀 <strong>大多数数据库系统的默认隔离级别</strong></p>
<h3 id="可重复读-repeatable-read-rr" tabindex="-1"><a class="header-anchor" href="#可重复读-repeatable-read-rr" aria-hidden="true">#</a> 可重复读(repeatable read，RR)</h3>
<blockquote>
<p>可重复度，是指同一个事务执行过程看到的数据，总是跟这个事务在启动时看到的数据是一致的。即同一个事务多次请求读取数据，会看到同样的数据行。</p>
</blockquote>
<p>👀 <strong>MySQL的默认隔离级别</strong></p>
<h3 id="串行化-serializable-s" tabindex="-1"><a class="header-anchor" href="#串行化-serializable-s" aria-hidden="true">#</a> 串行化(serializable，S)</h3>
<blockquote>
...
<h3 id="mvcc下sql语句实际处理" tabindex="-1"><a class="header-anchor" href="#mvcc下sql语句实际处理" aria-hidden="true">#</a> MVCC下SQL语句实际处理</h3>
...
<figure><img src="@source/database/mysql/images/row-version.png" alt="行-版本" tabindex="0" loading="lazy"><figcaption>行-版本</figcaption></figure>
...
<figure><img src="@source/database/mysql/images/read-view.png" alt="视图" tabindex="0" loading="lazy"><figcaption>视图</figcaption></figure>
...
<p>示例：</p>
<div class="language-sql line-numbers-mode" data-ext="sql"><pre v-pre class="language-sql"><code><span class="token keyword">create</span> <span class="token keyword">table</span> <span class="token identifier"><span class="token punctuation">`</span>t<span class="token punctuation">`</span></span><span class="token punctuation">(</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们以<strong>可重复读隔离级别</strong>为例：</p>
<table>
<thead>
<tr>
...
<p>这里注意：</p>
<p><code v-pre>begin/start transaction</code> 命令并不是一个事务的起点，在执行到它们之后的第一个操作 InnoDB 表的语句，事务才真正启动。如果你想要马上启动一个事务，可以使用 <code v-pre>start transaction with consistent snapshot</code>这个命令。</p>
...
<div class="language-sql line-numbers-mode" data-ext="sql"><pre v-pre class="language-sql"><code><span class="token keyword">select</span> <span class="token operator">*</span> <span class="token keyword">from</span> <span class="token keyword">table</span> <span class="token keyword">where</span> ?；
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="当前读" tabindex="-1"><a class="header-anchor" href="#当前读" aria-hidden="true">#</a> 当前读</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们以<strong>读提交隔离级别</strong>为例：</p>
<p>由于<code v-pre>start transaction with consistent snapshot;</code>在读提交隔离级别下没有意义，等价于普通的<code v-pre>start transaction</code></p>
<table>
<thead>
<tr>
<th style="text-align:center">事务A</th>
...
<p>事务A在执行<code v-pre>select k from t where id=1;</code>才创建一致性视图，此时事务C已提交，因此k=2;</p>
<p>同样事务B，事务C已提交，因此可见，且自己修改的肯定可见，因此k=3;</p>
<h2 id="那么mysql可重复读解决幻读了吗" tabindex="-1"><a class="header-anchor" href="#那么mysql可重复读解决幻读了吗" aria-hidden="true">#</a> 那么MySQL可重复读解决幻读了吗？</h2>
<p>我们试验为例：</p>
<p>假设存在表<code v-pre>dept</code></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>mysql<span class="token operator">></span> desc dept<span class="token punctuation">;</span>
+-------+-------------+------+-----+---------+-------+
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>开启两个事务进行试验</p>
<table>
<thead>
<tr>
<th style="text-align:center">事务A</th>
...
<p>可以看到通过对<code v-pre>id&gt;=1</code>加锁，成功解决幻读问题，但是如果在间隙锁之外进行数据操作，其实还是会出现幻读。因此MySQL可重复读隔离级别通过间隙锁部分解决幻读问题，并没有完全解决，如果要解决幻读问题只能使用串行化隔离级别。</p>
</div></template>
