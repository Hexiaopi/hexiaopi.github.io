...
公司目前对外部署的应用基本上都是使用Docker和Docker-Compose，当然也包括数据库，例如：MySQL、Postgresql。如何减少频繁的拷贝SQL脚本到数据库容器中这种繁琐的操作，提高部署效率呢？

<!-- more -->

Docker其实提供`docker-entrypoint-initdb.d`这一个特殊的目录，专门用于MySQL或Postgresql等数据库用于初始化数据。

## 使用方式

我们以MySQL为例，提供`docker-entrypoint-initdb.d`的使用方式。
...
在上面的使用示例中，我们将`./config/mysql/initdb`目录挂载进容器内：`/docker-entrypoint-initdb.d`目录，我们只需要在`./config/mysql/initdb/`目录下准备数据脚本，支持`.sh`和`.sql`两种数据脚本，也支持一些压缩的sql文件。

## 原理

那么为什么提供一个目录就可以初始化数据了呢？

这是由于Docker官方镜像启动时会执行entrypoint.sh脚本。我们可以访问一个MySQL容器，查看
